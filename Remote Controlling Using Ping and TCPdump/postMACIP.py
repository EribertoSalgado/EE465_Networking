...
import re #Used to extract MAC out of ifconfig
...
def get_ip_address():
    ...
    try:
        s.connect(("8.8.8.8", 80)) #connects to googles public DNS server to determine Rpi IP
...
def get_mac_address():
    ...
    try:
        ...
        ifconfig_result = subprocess.check_output("ifconfig", shell=True).decode() #get the data
        mac_address = re.search(r"ether ([\da-fA-F:]{17})", ifconfig_result).group(1) #extract
    except Exception:
...
def send_ip_mac():
    ...
    subprocess.run(command, shell=True) #run
